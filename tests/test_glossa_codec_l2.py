...
def test_l2_decode_raises_on_codebook_version_skew():
    ...
    raw = codec.encode(
        Message(intent="coord.claim", args={"task": "abc"}), codec.L2_CODEBOOK, sender_cb
    )
...
def test_l2_roundtrip_with_unicode_in_args_and_text():
    cb = default_codebook()
    m = Message(intent="coord.claim", args={"name": "你好"}, refs=["t1"], text="café 🔥")
    out = codec.decode(codec.encode(m, codec.L2_CODEBOOK, cb), codec.L2_CODEBOOK, cb)
    assert out == m
...
def test_l2_denser_than_l1_for_unknown_and_big_args():
    ...
    assert len(codec.encode(unknown, codec.L2_CODEBOOK, cb)) < len(
        codec.encode(unknown, codec.L1_SCHEMA)
    )
    # big-args known intent
    big = Message(intent="coord.claim", args={f"k{i}": "v" * 20 for i in range(20)})
    assert len(codec.encode(big, codec.L2_CODEBOOK, cb)) < len(codec.encode(big, codec.L1_SCHEMA))

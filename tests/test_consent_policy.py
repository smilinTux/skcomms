...
def test_user_allow_beats_server_block(home):
    ...
    p = InvitePolicy("lumina", enabled=True, allowed_users={L}, blocked_servers={"chef.skworld.io"})
    assert p.evaluate(L) == "allow"
...
def test_user_block_beats_server_allow(home):
    ...
    p = InvitePolicy("lumina", enabled=True, blocked_users={O}, allowed_servers={"chef.skworld.io"})
    assert p.evaluate(O) == "block"
    assert p.evaluate(L) == "allow"


def test_user_ignore_beats_server_allow(home):
    p = InvitePolicy("lumina", enabled=True, ignored_users={O}, allowed_servers={"chef.skworld.io"})

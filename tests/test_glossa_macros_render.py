...
def test_expand_macros_is_single_pass_no_reexpansion():
    ...
    lex = MacroLexicon({"ship": "rebase onto main then push", "main": "MAIN-BRANCH-REF"})
    ...
    assert (
        expand_macros("ship and main", lex) == "(rebase onto main then push) and (MAIN-BRANCH-REF)"
    )
...
def test_expand_macros_prefers_longest_match_first():
    ...
    lex = MacroLexicon({"P0": "priority zero", "P0 down": "priority-zero outage"})

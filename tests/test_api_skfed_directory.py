...
def _cluster_json(tmp_path_factory):
    """The announce endpoint's upsert_entry() mints a brand-new directory's
    realm/operator via the strict require_realm()/require_operator() (coord
    card 076d49cd) when no directory exists yet, so tests need a real,
    deterministic cluster.json regardless of the host's real one."""

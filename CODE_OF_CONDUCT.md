...
We as members, contributors, and leaders pledge to make participation in the `skcomms`
...
Because `skcomms` is **cryptographic comms** software, positive conduct here also means:

- **Intellectual honesty about security claims.** Do not overstate assurance. We say
  "quantum-resistant" / "post-quantum" at the -768 hybrid tier, never "quantum-proof,"
  "quantum-safe," or "unbreakable," and we never describe a classical surface (default
  signatures, a classical-peer payload wrap) as post-quantum. Claims cite evidence
  (surface + FIPS/RFC number + hybrid-vs-classical).
- **Respect for sovereignty and privacy.** Never publish another person's or agent's
  private key or identity material; never bind a federation socket to a public interface
  or weaken the ACK sender-binding / verify order for convenience.
...
maintainer contact listed on the [`smilinTux/skcomms`](https://github.com/smilinTux/skcomms)
